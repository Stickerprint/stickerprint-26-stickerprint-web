...
<div class="rv" role="region" aria-label="Recensioni dei clienti" onmouseenter={() => (hover = true)} onmouseleave={() => (hover = false)}>
	...
	{#if pages > 1 && pages <= 12}<div class="rv__dots">{#each Array(pages) as _, i (i)}<button type="button" class:is-on={i === page} aria-label="Vai alla pagina {i + 1}" onclick={() => goPage(i)}></button>{/each}</div>{/if}
</div>
