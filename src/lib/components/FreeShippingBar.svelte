<script lang="ts">
	...
	import { FREE_SHIPPING_GROSS, SHIPPING_GROSS, missingForFree } from '$lib/shipping-rules';
...
<div class="fship" class:fship--ok={missing === 0} class:fship--compact={compact} role="status">
	...
	{#if !compact}<small>Spedizione gratuita in Italia da {eur(FREE_SHIPPING_GROSS)} · sotto la soglia {eur(SHIPPING_GROSS)}</small>{/if}
</div>
