<script lang="ts">
	...
	type StudioExport = { blob?: Blob; glossD?: string; zone?: number; nodi?: number; soggetti?: { pathD: string; x: number; y: number; w: number; h: number; nodi: number }[]; foglio?: { w: number; h: number }; unione?: number; cutW?: number; cutH?: number; bleed?: number; pathD?: string; polys?: [number, number][][] | null; name?: string | null; shape?: string; dpi?: number; border?: number; stato?: Record<string, unknown> };
	const pending = new Map<string, { ok: (v: StudioExport) => void; ko: (e: Error) => void }>();
	export function studio(what: 'mockup' | 'print' | 'geom' | 'stato' | 'applica' | 'soggetti' | 'rilievo', opts: Record<string, unknown> = {}): Promise<StudioExport> {
