<script lang="ts">
	...
	function reorder() {
		addToCart({ product: order.product_slug, forma: order.forma ?? 'sagomato', materiale: order.materiale ?? 'bianco', w, h, qty: n, gross: q.gross, fileName: null, note: `Riordino ordine ${order.number}${order.finitura ? ' · ' + order.finitura : ''}` });
		added = true;
	}
