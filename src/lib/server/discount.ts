...
export async function checkDiscount(supabase: SupabaseClient, code: string, subtotalNet: number, email?: string | null): Promise<DiscountResult | { ok: false; error: string }> {
	...
	if (data.bound_email && email && data.bound_email.toLowerCase() !== email.trim().toLowerCase()) return { ok: false, error: 'Questo codice è personale: usa l\'email a cui è stato inviato.' };
