 ...
 *      UV        -> adesivi SENZA plastifica protettiva e adesivi in rilievo;
 *      resinati  -> tutti i resinati;
 *      laminati  -> gli adesivi che vogliono la plastifica.
 *  - OTTIMIZZAZIONE: sulla stessa bobina vanno tutti i lavori con la STESSA plastifica, cosi' si fa
 *    una stampa sola e una plastificata sola. Se di quella plastifica c'e' un lavoro solo si aspetta
 *    un giorno per vedere se ne arrivano altri; al secondo giorno si stampa comunque.
 *    Un ordine arrivato dopo, con la stessa plastifica, entra nella bobina gia' aperta.
...
export const RUOLO: Record<Ruolo, { label: string; cosa: string; icon: string }> = {
	uv: { label: 'Stampa UV', cosa: 'adesivi senza plastifica e rilievo', icon: '🟣' },
	resinati: { label: 'Resinati', cosa: 'tutti i resinati', icon: '💧' },
	laminati: { label: 'Con plastifica', cosa: 'adesivi da plastificare', icon: '🧴' }
};

/** margine da tenere sulla data di consegna, in giorni lavorativi */
export const MARGINE = { sito: 1, manuale: 2 };
/** giorni di attesa massima per mettere insieme altri lavori con la stessa plastifica */
...
export interface LavoroStampa {
	...
	cliente: string;
	prodotto: string;
	/** plastifica richiesta: 'nessuna' oppure lucida, opaca, … */
	protezione: string;
	rilievo: boolean;
	resinato: boolean;
...
export interface Bobina {
	id: string;
	ruolo: Ruolo;
	/** plastifica della bobina ('nessuna' sulla UV) */
	protezione: string;
	lavori: LavoroStampa[];
	mq: number;
	minuti: number;
...
/**
 * I lavori diventano bobine: una per macchina e per tipo di plastifica.
...
export function bobine(lavori: LavoroStampa[], oggi: string, cal: Calendar): Bobina[] {
	...
	for (const b of out) {
		...
		if (b.inCorso) { b.pronta = true; b.motivo = 'stampa già avviata'; continue; }
		if (b.avviaEntro && b.avviaEntro <= oggi) {
			b.pronta = true;
			b.motivo = b.avviaEntro < oggi ? 'in ritardo: va stampata subito' : 'ultimo giorno utile per la consegna';
			continue;
		}
		if (b.lavori.length > 1) { b.pronta = true; b.motivo = `${b.lavori.length} lavori con la stessa plastifica: una stampata sola`; continue; }
		if (b.attesaGiorni >= ATTESA_MAX) { b.pronta = true; b.motivo = `aspetta da ${b.attesaGiorni} giorni: si stampa da sola`; continue; }
		b.pronta = false;
		const restano = ATTESA_MAX - b.attesaGiorni;
		b.motivo = `lavoro solo: aspetto ancora ${restano} ${restano === 1 ? 'giorno' : 'giorni'} un altro con la stessa plastifica`;
	}
