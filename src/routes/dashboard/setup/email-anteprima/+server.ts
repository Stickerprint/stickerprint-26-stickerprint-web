...
export const GET: RequestHandler = async ({ url, locals: { user } }) => {
	...
	const ALL: Record<string, () => { subject: string; html: string; tag?: string }> = {
		...
		consegnato: () => T.shippingUpdateEmail({ ...ship, kind: 'consegnato' }),
		...
		file_mancante: () => T.proofReminderEmail({ name: 'Mattia', number: 'SP00037', missingFile: true, approveBy: new Date(Date.now() + 86400000), shipBy: '2026-09-22', href: `${o}/account/ordini` }),
		preventivo: () => T.quoteEmail({ subject: 'Il tuo preventivo SPP00001 è pronto', message: staffMsg('il preventivo per le 1.000 etichette in bobina'), senderName: 'Mattia', number: 'SPP00001', validUntil: '30 settembre 2026', href: `${o}/preventivo/esempio` }),
		...
		preventivo_accettato: () => T.quoteAcceptedEmail({ name: 'Mattia', number: 'SPP00001', orderNumber: 'SP00038' }),
