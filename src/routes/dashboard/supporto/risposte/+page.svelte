...
{#if adding}
	<form method="POST" action="?/salva" use:enhance={() => async ({ update }) => { await update(); adding = false; }} class="dcard hd-tpl">
		...
		<textarea name="body" rows="4" placeholder="Testo della risposta…" required></textarea>
	...
	{#each data.templates as t (t.id)}
		...
		{:else}
			<div class="dcard" style="display:grid;gap:6px">
				<div class="toolbar" style="justify-content:space-between"><b>{t.title}</b><div class="toolbar" style="gap:6px"><button class="btn btn--ghost btn--xs" type="button" onclick={() => (editing = t.id)}>Modifica</button><form method="POST" action="?/elimina" use:enhance><input type="hidden" name="id" value={t.id} /><button class="btn btn--ghost btn--xs" type="submit">🗑️</button></form></div></div>
