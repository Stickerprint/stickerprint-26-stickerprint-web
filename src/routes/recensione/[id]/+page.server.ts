...
export const load: PageServerLoad = async ({ params, url }) => {
	...
	const { data: items } = await db.from('orders').select('id, product_name, qty').eq(o.checkout_group ? 'checkout_group' : 'id', o.checkout_group ?? o.id);
	/* una recensione per ordine: se c'e' gia' per uno qualsiasi degli articoli, e' fatta */
	const { data: existing } = await db.from('reviews').select('id, rating, coupon_code').in('order_id', (items ?? []).map((i) => i.id)).limit(1);
	const ex = existing?.[0];
	return { number: o.number, name: o.shipping?.first_name ?? '', items: (items ?? []).map((i) => `${i.qty} × ${i.product_name}`), done: !!ex, code: ex?.coupon_code ?? null, requestId: url.searchParams.get('r') ?? '' };
};
...
export const actions: Actions = {
	default: async ({ request, params, url }) => {
		...
		const { data: group } = await db.from('orders').select('id, checkout_group').or(`id.eq.${params.id},checkout_group.eq.${params.id}`);
		const r = await submitReview(db, { orderId: o.id, userId: o.user_id ?? null, email: o.email ?? null, author, productSlug: o.product_slug ?? null, rating: Number(f.get('rating') ?? 5), title: String(f.get('title') ?? '').trim() || null, comment: String(f.get('comment') ?? '').trim() || null, number: o.number, origin: url.origin, groupOrderIds: (group ?? []).map((g) => g.id), checkoutGroup: group?.[0]?.checkout_group ?? null, requestId: String(f.get('r') ?? '') || null });
